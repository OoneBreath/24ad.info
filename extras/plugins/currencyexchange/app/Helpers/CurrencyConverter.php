...
class CurrencyConverter
{
	...
	public static function getRate(array $currencyFrom, array $currencyTo): float|int|null
	{
		...
		if (!empty($currencyFrom['rate']) && !empty($currencyTo['rate'])) {
			$rateFrom = (float)Num::toFloat($currencyFrom['rate']);
			$rateTo = (float)Num::toFloat($currencyTo['rate']);
	...
	public static function convert(float|int $amount, string $currencyFrom, string $currencyTo, int $precision = 2): float
	{
		...
		if (
			...
			&& !empty($rates[$currencyFrom]['rate'])
			&& !empty($rates[$currencyTo]['rate'])
		) {
			$rateFrom = (float)Num::toFloat($rates[$currencyFrom]['rate']);
			$rateTo = (float)Num::toFloat($rates[$currencyTo]['rate']);
	 ...
	 * @return bool
	 */
	public static function updateCurrenciesRates(): bool
	{
		try {
