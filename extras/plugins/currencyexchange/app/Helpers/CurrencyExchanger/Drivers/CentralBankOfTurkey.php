...
class CentralBankOfTurkey extends AbstractDriver
{
	...
	private function formatDataUsingRegex($data, string $currencyBase): array
	{
		...
		$rates = collect($matches)
			->mapWithKeys(function ($item) use ($date) {
				$code = $item[1];
	...
	private function formatDataUsingSimplexml($data, string $currencyBase): array|string
	{
		...
		$rates = collect($phpArray)
			->mapWithKeys(function ($item) use ($currencyBase, &$date) {
				if (!empty($item['Tarih'])) {
					$date = $item['Tarih'];
				}
				if (empty($date)) {
				...
				$rates = collect($item)
					->mapWithKeys(function ($v) use ($date) {
						if (
							!isset($v['@attributes'])
						...
						$date = (!empty($date) && !str_contains($date, '1970') && str_starts_with($date, date('Y')))
							? $date
							: date('Y-m-d H:i:s');
