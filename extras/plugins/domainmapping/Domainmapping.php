 ...
 * 2. Administrators: Related to the #1, by installing the Domain Mapping plugin,
 *    you have to set a domain (or subdomain) per country to prevent any SEO issue.
...
use extras\plugins\domainmapping\database\MigrationsTrait;
...
class Domainmapping
{
	...
	public static function install(): bool
	{
		...
		try {
			...
			$res = createPluginSetting($pluginSetting);
			if (!$res) {
				return false;
			}
	...
	private static function uninstallWithoutLogout(): bool
	{
		try {
			...
			dropPluginSetting('domainmapping');
