...
trait SecurityConfig
{
	private function updateSecurityConfig(?array $settings = []): void
	{
		...
		if (config('domainSettings.security.captcha') == 'recaptcha') {
			$version = config('domainSettings.security.recaptcha_version', 'v2');
			$version = env('RECAPTCHA_VERSION', $version);
			
			if ($version == 'v3') {
				$siteKey = config('domainSettings.security.recaptcha_v3_site_key');
				$secretKey = config('domainSettings.security.recaptcha_v3_secret_key');
			} else {
				$siteKey = config('domainSettings.security.recaptcha_v2_site_key');
				$secretKey = config('domainSettings.security.recaptcha_v2_secret_key');
			}
			
			$skipIps = env('RECAPTCHA_SKIP_IPS', config('domainSettings.security.recaptcha_skip_ips', ''));
